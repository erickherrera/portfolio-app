...
const SunIcon = ({ isDarkMode, className }) => (
  <svg 
    ...
    fill="none" 
    strokeWidth={1.5} 
    stroke={isDarkMode ? "#171717" : "#ededed"}
    className={className}
  >
...
export default function ThemeToggle() {
  ...
  // Use dynamic background colors based on theme and hover state
  const getButtonStyle = () => {
    if (resolvedTheme === THEMES.DARK) {
      return {
        backgroundColor: isHovered ? colors.secondary : colors.foreground,
        transition: 'background-color 0.3s'
      };
    } else {
      return {
        backgroundColor: isHovered ? colors.hero : colors.secondary,
        transition: 'background-color 0.3s'
      };
    }
  };
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className="relative p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
      ...
      >
        ...
        <div className={getIconClasses(THEMES.LIGHT)}>
          <SunIcon isDarkMode={resolvedTheme === THEMES.DARK} className="w-full h-full" />
        </div>
        
        {/* Dark mode icon */}
        <div className={getIconClasses(THEMES.DARK)}>
          <MoonIcon isDarkMode={resolvedTheme === THEMES.DARK} className="w-full h-full" />
        </div>
        
        {/* System preference icon */}
        <div className={getIconClasses(THEMES.SYSTEM)}>
          <ComputerIcon isDarkMode={resolvedTheme === THEMES.DARK} className="w-full h-full" />
        </div>
        
        {/* System theme indicator */}
        {theme === THEMES.SYSTEM && (
          <div className="absolute -top-1 -right-1 w-2 h-2 rounded-full" 
            style={{ backgroundColor: resolvedTheme === THEMES.DARK ? '#171717' : '#ededed' }} 
            aria-hidden="true" 
          />
        )}
